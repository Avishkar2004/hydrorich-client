...
function FungicideList() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch(API_ENDPOINTS.products.fungicides);
                const data = await res.json();
                setFungicides(data.products);
                setFilteredProducts(data.products);
            } catch (err) {
                setError('Failed to load Fungicide products.');
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="px-4 sm:px-8 md:px-12 py-10 bg-gray-50 min-h-screen">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredProducts.map((product) => {
                    ...
                    return (
                        <Link to={`/fungicides/${product.id}`} key={product.id}>
